---
name: metrobuildsuppliers.com.au client profile
description: Metrobuild Building & Construction Suppliers — trade-focused building supplies store in Springvale, Melbourne, keyword plan v1 complete 7 May 2026
type: project
---

# Metrobuild Building & Construction Suppliers Pty Ltd — Client Profile

## Identity
- **Domain:** metrobuildsuppliers.com.au
- **Business name:** Metrobuild Building & Construction Suppliers Pty Ltd
- **ABN:** 21 691 273 775
- **Founded:** October 2025
- **Phone:** 0375777976
- **Address:** 6/865A Princes Highway, Springvale VIC
- **Email:** info@metrobuildsuppliers.com.au
- **Hours:** 5:30AM–6:00PM MON–FRI, 6:00AM–2:00PM SAT–SUN (open 7 days)
- **Website:** https://metrobuildsuppliers.com.au
- **Owners:** Gurjeet Singh, Mehmet Ali Ozkavak

## Business Type
**Local trade supply store — Melbourne, B2B/trade-focused**
- Physical store in Springvale (South-East Melbourne)
- Target: tradies, renderers, plasterers, tilers, waterproofers, cladders, builders, subcontractors
- NOT residential/DIY. Trade and professional contractors primary audience.
- Opened October 2025 — very new business

## Core Product Categories
| Category | Brands/Products |
|----------|----------------|
| AAC (Autoclaved Aerated Concrete) panels | Hebel, Nasahi, Eastland |
| Fibre cement cladding | James Hardie, AFS |
| EPS (Expanded Polystyrene) systems | NRG Greenboard, Statewall |
| External rendering systems | Rockcote, Unitex, cement render, polymer render, acrylic texture |
| Decorative finishes | Venetian Plaster, Micro Cement |
| Tiling & waterproofing products | Tiling adhesives, waterproofing membranes |
| PPE products | On-site safety equipment |
| Wall sarking | Wall wrap/sarking systems |

## Core Services
- 7-day in-store pickups with payment facility
- Product display showrooms (hands-on)
- Workshops and training for tradespeople
- Same-day delivery (Ute, Tray Truck, Crane & Fork Truck — crane & forklift capability is a USP for heavy AAC panels)

## USPs
1. Hands-on showroom with real product displays (not just a catalogue)
2. Official distributor of Hebel, James Hardie, NRG, Rockcote, Unitex, etc.
3. Open 7 days (early start 5:30AM for tradies)
4. Crane & forklift delivery — can handle heavy AAC/Hebel panels
5. Workshops & education — fills a gap in trade knowledge for cladding/coating systems
6. Specialists: rendering + waterproofing + tiling + cladding (multi-trade coverage)
7. Price matching + bulk pricing + loyalty rewards

## Competitors Named by Client
- Archiway (DA 22)
- Bell Plaster (DA 28) — most direct render competitor
- Bowens (DA 52) — unbeatable on generic terms
- Crowntex (DA 18) — weakest competitor
- National Tiles (DA 48) — consumer tile focus, not trade

## Target Customers
1. Solid plasterers / renderers
2. Tilers
3. Waterproofers
4. Cladders
5. Builders
6. Subcontractors

## Keyword Research Scope
- **15 General pages + 10 Location pages**
- Completed: 7 May 2026

## Keyword Plan v1 — Summary
- **15 general pages** covering rendering (3 pages), cladding/AAC (4 pages), waterproofing+tiling (2 pages), decorative finishes (2 pages), ancillary (2 pages), hub (1 page), content pillar (1 page)
- **10 location pages** — "rendering supplies {suburb}" silo: Dandenong, Glen Waverley, Clayton, Keysborough, Noble Park, Cranbourne, Berwick, Truganina, Craigieburn, Richmond
- **Effective reach:** 1,640/mo primary keyword volume
- **Top keyword:** building supplies melbourne (390/mo, KD 34 — 12+ month strategic anchor)
- **Quickest win:** micro cement supplier melbourne (KD 9, DA 12 at #1 — 2-3 month)
- **Highest CPC:** james hardie cladding supplier ($5.20)
- **Client DA:** ~4 (new domain)

## Deliverable
- `/home/invoi/fahad_projects/clients/metrobuildsuppliers.com.au/keyword-research/keyword-plan.html` (v1, 7 May 2026)

## Key Flags
- ⚠️ Business only 7 months old (Oct 2025) — new domain, minimal DA expected
- "CSR cladding system" in Q13 — CSR is now Dulux/Hebel; Hebel is CSR's brand. Covered under Hebel page.
- Venetian Plaster and Micro Cement are decorative/luxury finish products — slightly different audience (interior designers, high-end builders)
- NRG Greenboard = EPS render board (NRG Insulated Panel Systems)
- Wall sarking = underlay/wrap behind cladding — niche product
- Springvale = home suburb → homepage, NOT a standalone location page
- PPE is Tier D — not a differentiator, covered as secondary on store page
